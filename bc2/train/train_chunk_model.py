...
def train():
    ...
    dataset = load_dataset(os.path.splitext(os.path.basename(data_args.data_path))[1][1:], data_files=data_args.data_path,num_proc=6, split="train")# train[:15%]
    if training_args.slide_window:
        train_dataset = dataset
    else:
        train_dataset = dataset.map(tokenize_function, fn_kwargs={"tokenizer": tokenizer, "max_len":training_args.model_max_length},num_proc=6)

    eva_data ={}
    for evd in data_args.eval_data_path:
        dataset=load_dataset(os.path.splitext(os.path.basename(evd))[1][1:], data_files=evd,num_proc=6, split='train')
        eval_dataset=dataset.map(tokenize_function, fn_kwargs={"tokenizer": tokenizer, "max_len":training_args.model_max_length},num_proc=6)
        eva_data[os.path.splitext(os.path.basename(evd))[0]] = eval_dataset
    
    if training_args.slide_window:
        trainer = Mytrainer(model=model, tokenizer=tokenizer, args=training_args, train_dataset=train_dataset,eval_dataset=eva_data)
    else:  
