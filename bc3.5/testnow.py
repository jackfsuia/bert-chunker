...
model_path = r"D:\github-my-project\bert-chunker-4\model_checkpoint\checkpoint-1200"
...
testing_text = textcvx_plus*30
with torch.no_grad():

    unk_id = tokenizer.unk_token_id
    
    tokens = tokenizer(testing_text, return_tensors="pt", truncation=False)
...
def find_split_points2(numbers, M):
    """
    完全O(N)时间复杂度的解决方案
    ...
    while i <= n - M:
        ...
        for j in range(i, i + M):
            # 维护递减队列
            while dq and numbers[dq[-1]] <= numbers[j]:
                dq.pop()
            dq.append(j)
...
substrings = [
    testing_text[i:j] for i, j in zip(str_split_points, str_split_points[1:] + [len(testing_text)])
]
t2 = time.time()
print("time:",t2-t1)
for i, (c, t) in enumerate(zip(substrings, token_split_points)):
