...
def train():
    ...
    # dataset = load_dataset("json", data_files=data_args.data_path, num_proc=6,split="train")# train[:15%]
    train_dataset = load_dataset(os.path.splitext(os.path.basename(data_args.data_path))[1][1:], data_files=data_args.data_path,num_proc=6, split="train")# train[:15%]
    if training_args.slide_window or training_args.API_syn_dataset:
        train_dataset = train_dataset
        eva_data ={}
        for evd in data_args.eval_data_path:
            eval_dataset=load_dataset(os.path.splitext(os.path.basename(evd))[1][1:], data_files=evd,num_proc=6, split='train')
            eva_data[os.path.splitext(os.path.basename(evd))[0]] = eval_dataset
    else:
        train_dataset = train_dataset.map(tokenize_function, fn_kwargs={"tokenizer": tokenizer, "max_len":training_args.model_max_length},num_proc=6)
        eva_data ={}
        for evd in data_args.eval_data_path:
            eval_dataset=load_dataset(os.path.splitext(os.path.basename(evd))[1][1:], data_files=evd,num_proc=6, split='train')
            eval_dataset=eval_dataset.map(tokenize_function, fn_kwargs={"tokenizer": tokenizer, "max_len":training_args.model_max_length},num_proc=6)
            eva_data[os.path.splitext(os.path.basename(evd))[0]] = eval_dataset

    
    if training_args.slide_window:
        trainer = deterSampleTrainer(model=model, args=training_args, train_dataset=train_dataset,eval_dataset=eva_data)
    else:  
        trainer = Trainer(model=model,args=training_args, train_dataset=train_dataset,eval_dataset=eva_data)
