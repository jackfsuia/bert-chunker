...
class BertChunker(PreTrainedModel):
    ...
    def chunk_text(self, text: str, tokenizer, prob_threshold=0.5) -> list[str]:
        ...
        while windows_end <= input_ids.shape[1]:
            ...
            output = self(
                input_ids=ids,
                attention_mask=torch.ones(1, ids.shape[1], device=self.device),
            )
        ...
        substrings = [
            text[i:j]
            for i, j in zip([0] + split_str_poses, split_str_poses + [len(text)])
        ]
    ...
    def chunk_text_smooth(self, text: str, tokenizer, threshold=0) -> list[str]:
        ...
        for i in range(len(prob_pair_list)):
            ...
            if math.prod(prob_pair_list[i])/len(prob_pair_list[i])>threshold:
                split_str_poses+=[tokens.token_to_chars(i + 1).start]
            # all_in = 1
            # for num in prob_pair_list[i]:
            #     if num<threshold:
            #         all_in = 0
            #         break
            # if all_in:
